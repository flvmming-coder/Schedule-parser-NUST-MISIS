...
9. For worldwide access, choose `Защищенный канал` or open access, set the browser password if needed, click `Опубликовать в интернет` in `ScheduleDepartmentApp`, and use the GitHub Pages URL.
...
The LAN server is still useful inside one local network. For access from any city, provider, phone network, Wi-Fi, or Windows device, use the global GitHub Pages URL below. In protected mode the browser starts at the welcome page, opens the schedule page only after password entry, and `schedule.json` is published as an encrypted package; in open mode it is published as ordinary JSON.
